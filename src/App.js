...
export default function App() {
  ...
  return (
    <div>
      <aside id="default-sidebar" className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 sm:${isSideBar ? 'block' : 'hidden'}`} aria-label="Sidebar">
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          <ul className="space-y-2 font-medium">
            <li onClick={() => setIsSideBar(false)}>
      ...
      <div className={`p-1 sm:ml-${isSideBar ? 64 : 100}`}>
        <div className="p-4 border-2 border-gray-200  rounded-lg dark:border-gray-700">
          
          <ul className="hidden text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg shadow sm:flex dark:divide-gray-700 dark:text-gray-400">
            <li className="w-full" onClick={() => setIsSideBar(!isSideBar)}>
              <a href="#" className="inline-block w-full p-4 text-gray-900 bg-gray-100 rounded-l-lg focus:ring-4 focus:ring-blue-300 active focus:outline-none dark:bg-gray-700 dark:text-white" aria-current="page">{isSideBar?'Close':'Open'}</a>
            </li>
            <li className="w-full">
